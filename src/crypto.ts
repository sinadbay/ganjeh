...
export interface EncryptedEnvelope {
  ...
  readonly cipher: typeof CIPHER_ALGORITHM;
  readonly kdf: KdfBlock;
  /** base64, exactly `SALT_LEN` bytes once decoded */
  readonly saltB64: string;
  /** base64, exactly `NONCE_LEN` bytes once decoded */
  readonly nonceB64: string;
  /** base64; the AES-GCM auth tag is appended to the ciphertext bytes */
  readonly ciphertextB64: string;
}

/**
 * The contract this module fulfils. `src/types.ts` — described in this node's
 * brief as an already-landed prerequisite carrying the shared `Cipher`
 * interface and error taxonomy — does not exist anywhere in this repository's
 * history. This local declaration stands in for it; the shape is what the
 * brief describes, so the exported `cipher` object will satisfy the real
 * thing unchanged once it lands.
 *
...
export interface Cipher {
  seal(plaintext: Buffer, passphrase: string): Promise<EncryptedEnvelope>;
  open(envelope: EncryptedEnvelope, passphrase: string): Promise<Buffer>;
}
 ...
 */
export const KDF_PARAMS = { name: 'scrypt', n: 131072, r: 8, p: 1 } as const;
 ...
 */
export function validateKdfParams(input: unknown): KdfBlock {
  ...
  if (name !== 'scrypt') {
    throw new VaultCorruptError(
      `unsupported kdf.name ${describeUntrusted(name)}; expected "scrypt"`,
    );
  }
  ...
  if (typeof n !== 'number' || !Number.isInteger(n) || n < MIN_N || n > MAX_N) {
    throw new VaultCorruptError(
      `kdf.n must be an integer between ${MIN_N} and ${MAX_N}, got ${describeUntrusted(n)}`,
    );
  }
 ...
 */
function deriveKey(passphrase: string, saltB64: string, params: unknown): Promise<Buffer> {
 ...
 */
export async function seal(plaintext: Buffer, passphrase: string): Promise<EncryptedEnvelope> {
 ...
 * came from `seal`.
 */
export async function open(envelope: EncryptedEnvelope, passphrase: string): Promise<Buffer> {
  if (typeof passphrase !== 'string') {
  ...
  if (cipherName !== CIPHER_ALGORITHM) {
    throw new VaultCorruptError(
      `unsupported cipher ${describeUntrusted(cipherName)}; expected ${JSON.stringify(CIPHER_ALGORITHM)}`,
    );
  }
  ...
  if (combined.length < AUTH_TAG_LEN) {
    throw new VaultCorruptError(
      `ciphertextB64 must decode to at least ${AUTH_TAG_LEN} bytes, got ${combined.length}`,
    );
  }
