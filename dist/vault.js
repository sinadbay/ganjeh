...
import { VaultError, VaultCorruptError, InvalidNameError, InvalidSecretError, EntryExistsError, EntryNotFoundError, describeUntrusted, NAME_PATTERN, } from "./types.js";
export { VaultError, VaultCorruptError, InvalidNameError, InvalidSecretError, EntryExistsError, EntryNotFoundError, describeUntrusted, NAME_PATTERN, };
// ---------------------------------------------------------------------------
...
function assertValidSecret(secret) {
    ...
    if (byteLength > MAX_SECRET_BYTES) {
        throw new InvalidSecretError(`secret is ${byteLength} bytes, over the ${MAX_SECRET_BYTES} byte limit`);
    }
}
// ---------------------------------------------------------------------------
 ...
 */
export function addEntry(data, name, secret, { overwrite = false, now = () => new Date().toISOString() } = {}) {
...
export function getEntry(data, name) {
...
export function encodeVault(data) {
    return Buffer.from(JSON.stringify(data), 'utf8');
}
export function decodeVault(buf) {
    if (buf.length > MAX_DOCUMENT_BYTES) {
        throw new VaultCorruptError(`document is ${buf.length} bytes, over the ${MAX_DOCUMENT_BYTES} byte limit`);
    }
    let parsed;
    try {
...
function decodeEntry(value, key) {
